...
function loadDb() {
  try {
    ...
    console.log(`[Sagacidade RJ] data.json carregado — ${Object.keys(db.rooms).length} sala(s), ${db.logs.length} log(s)`);
  } catch (e) {
    if (e.code !== 'ENOENT') console.warn('[Sagacidade RJ] data.json ilegível, começando vazio:', e.message);
...
async function flush() {
  ...
  try {
    ...
    console.error('[Sagacidade RJ] falha ao gravar data.json:', e.message);
...
try {
  ...
  dataWritable = false;
  console.error('[Sagacidade RJ] AVISO: sem permissão de escrita em', ROOT, '—', e.message);
  console.error('[Sagacidade RJ] salas não serão salvas. Corrija com: chown/chmod na pasta do projeto.');
}

server.on('error', err => {
  if (err.code === 'EADDRINUSE') {
    console.error(`[Sagacidade RJ] porta ${PORT} já está em uso — o Rise provavelmente já está rodando.`);
    console.error(`[Sagacidade RJ] abra http://localhost:${PORT} no navegador (não abra o index.html direto).`);
    process.exit(1);
  }
  throw err;
});
...
server.listen(PORT, HOST, () => {
  console.log(`[Sagacidade RJ] http://localhost:${PORT}`);
  console.log(`[Sagacidade RJ] dados em ${DATA_FILE}`);
});

for (const sig of ['SIGINT', 'SIGTERM']) {
  process.on(sig, async () => {
    console.log('\n[Sagacidade RJ] encerrando, gravando data.json...');
